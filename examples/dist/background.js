(function (factory) {
    ...
    ***************************************************************************** */
    ...
    var MiniPizza = /** @class */ (function (_super) {
        __extends(MiniPizza, _super);
        function MiniPizza() {
            return _super !== null && _super.apply(this, arguments) || this;
        }
        MiniPizza.prototype._patch = function (vnode) {
            this.$vnode = vnode;
            this.$mounted && MiniPizza.send(this, 'UPDATE_COMPONENT', vnode);
        };
        MiniPizza.send = function (instance, type, data) {
            self.postMessage({
                id: instance.$$id,
    ...
    self.addEventListener('message', function (event) {
        ...
        switch (type) {
            case 'COMPONENT_CREATED':
                if (id == 0)
                    return;
                var context = instances[data.context];
            ...
            case 'COMPONENT_INVOKE':
                instance.$invoke.apply(instance, data);
        }
    });
    config.constructor = MiniPizza;
    var Page = function (options) {
        var instance = new MiniPizza(options);
        instances[instance.$$id = 0] = instance;
        instance.$mount();
        MiniPizza.send(instance, 'CREATE_PAGE', __assign(__assign({}, instance.$vnode), { context: null }));
    };
    ...
    var template$1 = "<user v-for=\"list\" :info=\"$item\" v-for-index=\"index\" v-key=\"$item.key\" @click=\"onClickItem\">\r\n  <span slot=\"name\">{{$item.name}}</span>\r\n  {{JSON.stringify($item.fav)}}\r\n</user>";
    ...
    var Users = {
        ...
        lifetimes: {
            mounted: function () {
                var _this = this;
                setTimeout(function () {
                    _this.users = _this.users.slice(2);
                }, 2000);
            }
        },
        methods: {
